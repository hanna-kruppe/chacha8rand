...
pub(crate) fn from_elems(elems: [u32; 4]) -> __m128i {
...
pub(crate) fn splat(x: u32) -> __m128i {
    // SAFETY: requires the sse2 target feature, which was detected via cfg.
    unsafe { _mm_set1_epi32(x as i32) }
}

pub(crate) fn add_u32(x: __m128i, y: __m128i) -> __m128i {
    // SAFETY: requires the sse2 target feature, which was detected via cfg.
    unsafe { _mm_add_epi32(x, y) }
}

pub(crate) fn xor(x: __m128i, y: __m128i) -> __m128i {
    // SAFETY: requires the sse2 target feature, which was detected via cfg.
    unsafe { _mm_xor_si128(x, y) }
}

pub(crate) fn shift_left_u32<const IMM8: i32>(x: __m128i) -> __m128i {
    // SAFETY: requires the sse2 target feature, which was detected via cfg.
    unsafe { _mm_slli_epi32::<IMM8>(x) }
}

pub(crate) fn shift_right_u32<const IMM8: i32>(x: __m128i) -> __m128i {
    // SAFETY: requires the sse2 target feature, which was detected via cfg.
    unsafe { _mm_srli_epi32::<IMM8>(x) }
}

pub(crate) fn storeu(x: __m128i, dest: &mut [u8; 16]) {
